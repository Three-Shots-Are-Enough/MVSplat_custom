...
class GaussianAdapter(nn.Module):
    ...
    def forward(
        ...
        if not torch.allclose(torch.det(c2w_rotations), torch.tensor(1.0, device=c2w_rotations.device), atol=1e-3):
            raise ValueError("src/model/encoder/common: Detected rotation matrices with determinant not close to 1. Please check the input extrinsics.")
