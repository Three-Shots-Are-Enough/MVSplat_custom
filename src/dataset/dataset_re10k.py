...
class DatasetRE10k(IterableDataset):
    ...
    def convert_poses(
        ...
        print("[DEBUG] print flattened camera matrix 2:", poses[:, :][0])
        ...
        print("MIMIMIMIMIMI",w2c[:, :3][0])
        
        # Verify that each rotation matrix has a determinant close to 1
        #print(w2c[:, :3, :3])
        det_w2c_rotations = torch.det(w2c[:, :3, :3][0])
        print(w2c[:, :3, :3][0])
        print("[DEBUG]: det_w2c_ratations [0]:", det_w2c_rotations)
